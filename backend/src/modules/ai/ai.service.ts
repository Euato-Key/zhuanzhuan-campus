...
export const AIService = {
  recognition: {
    async analyze(userId: number, request: AIRecognitionRequest): Promise<AIRecognitionResult> {
      ...
      if (mcpAvailable) {
        return this.analyzeWithMCP(request, aiCategories, imageUrls);
      } else {
        return this.analyzeWithoutMCP(request, aiCategories, imageUrls);
      }
    },
    ...
    async analyzeStream(
      ...
      return env.MCP_ENABLED
        ? this.analyzeStreamWithMCP(request, aiCategories, imageUrls, onEvent)
        : this.analyzeStreamWithoutMCP(request, aiCategories, imageUrls, onEvent);
    },
    ...
    async analyzeStreamWithMCP(
      ...
      if (identification.searchKeywords.length > 0) {
        ...
        try {
          const t2 = Date.now();
          searchResults = await this.executePhase2(kw);
      ...
      if (searchResults.length > 0) {
        const fetchUrls = await this.selectRelevantUrls(searchResults, identification, env.MCP_FETCH_MAX_PAGES);
        onEvent({ type: 'phase_start', phase: 'phase3', message: '开始抓取网页详情', urls: fetchUrls });
        try {
          const t3 = Date.now();
          fetchedPages = await this.executePhase3(searchResults, fetchUrls);
      ...
      if (phases.mcpUsed && (searchResults.length > 0 || fetchedPages.length > 0)) {
        ...
        try {
          const t4 = Date.now();
          const fusion = await this.executePhase4(identification, preliminaryData, searchResults, fetchedPages, categories);
    ...
    async analyzeWithMCP(
      ...
      const phase1Result = await this.executePhase1(request, categories, imageUrls);
      ...
      if (identification.searchKeywords.length > 0) {
        try {
          ...
          searchResults = await this.executePhase2(identification.searchKeywords);
      ...
      if (searchResults.length > 0) {
        try {
          ...
          const selectedUrls = await this.selectRelevantUrls(searchResults, identification, env.MCP_FETCH_MAX_PAGES);
          fetchedPages = await this.executePhase3(searchResults, selectedUrls);
      ...
      if (phases.mcpUsed && (searchResults.length > 0 || fetchedPages.length > 0)) {
        try {
          ...
          const fusionResult = await this.executePhase4(
            identification,
            preliminaryData,
            searchResults,
            fetchedPages,
  ...
  audit: {
    /**
     * AI审核单个商品
     * 查询商品信息 → 检查AI审核配置 → 构建Prompt → 调用AI → 解析结果 → 更新状态 → 通知卖家
     */
